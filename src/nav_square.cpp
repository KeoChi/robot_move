...
int main(int argc, char** argv)
{
  ...
  double goal_angle = 90*M_PI/180; //角度转换成弧度:deg*PI/180;
  ...
  for(int i = 0;i < 4;i++)//Loop once for each leg of the trip
  {
    ...
    double turn_angle = 0;
    while( (fabs(turn_angle + angular_tolerance) < goal_angle) && (ros::ok()) )
    {
        //Publish the Twist message and sleep 1 cycle
        cmdVelPub.publish(speed);
        loopRate.sleep();
